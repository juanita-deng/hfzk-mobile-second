...
export default class My extends Component {
	...
	async getUserInfo() {
		...
		if (status === 200) {
			...
			this.setState({
				isLogin: false, //token异常时修改登陆状态 400
			});
		}
	...
	logout = () => {
		Modal.alert('温馨提示', '确定要退出吗?', [
			{ text: '取消' },
			{
				text: '确定',
				onPress: async () => {
					//发送请求退出,删除token,清空用户信息,修改登陆状态
					const { status } = await http.post('/user/logout');
					if (status === 200) {
						removeToken();
						this.setState({
							isLogin: false,
	...
	render() {
		...
		const { isLogin, nickname, userInfo } = this.state;
		return (
			<div className={styles.my}>
				...
				<div className="title">
					...
					<div className="info">
						...
						<div className="user">
							<div className="name">{isLogin ? nickname : '游客'}</div>
